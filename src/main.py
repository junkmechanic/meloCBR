...
from src.casestructure import Case, Note

import random

def build_cases(query):
    with src.dbconnector.grant_connection() as dbconnect:
        for row in dbconnect.get_case(query):
            case = Case(*row)
            for note in ['signif_note', 'sig_note_m1', 'sig_note_m2',
                         'sig_note_p1', 'sig_note_p2', 'htreb', 'hmid', 'hbass']:
                if case.features[note] is not None:
                    nquery = "SELECT * FROM NOTEBASE WHERE NID = " + str(case.features[note])
                    case.features[note] = Note(*dbconnect.get_note(nquery).fetchone())
            for notes in ['other_notes', 'hother']:
                ...
                if case.features[notes] is not None:
                    ...
                    for n in notelist:
                        ...
                        newlist.append(Note(*dbconnect.get_note(query).fetchone()))
...
def similarity(inp_case, case):
    s_value = 0.2
    for note, w in {'sig_note_m1':0.8, 'sig_note_m2':0.5,\
